...
namespace VOD.API.Controllers
{
    ...
    public class DownloadsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<DownloadDTO>>> Get(int courseId, int moduleId, bool include = false)
        {
            try
            {
                var dtos = courseId == 0 ?
                    ...
                    await _db.GetAsync<Download, DownloadDTO>(m => m.CourseId.Equals(courseId) &&
        ...
        [HttpGet("{id:int}")]
        public async Task<ActionResult<DownloadDTO>> Get(int courseId, int moduleId, int id,
            bool include = false)
        {
            try
            {
                var dto = courseId == 0 ?
                    ...
                    await _db.SingleAsync<Download, DownloadDTO>(m => m.CourseId.Equals(courseId) &&
        ...
        public async Task<ActionResult<DownloadDTO>> Post(int courseId, int moduleId, DownloadDTO model)
        {
            try
            {
                ...
                var uri = _linkGenerator.GetPathByAction("Get", "Downloads",
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Put(int courseId, int moduleId, int id, DownloadDTO model)
        {
            try
            {
        ...
        public async Task<IActionResult> Delete(int courseId, int moduleId, int id)
        {
            try
            {
                var exists = await _db.AnyAsync<Download>(a => a.CourseId.Equals(courseId)
                    && a.ModuleId.Equals(moduleId) && a.Id.Equals(id));
                if (!exists) return BadRequest("Could not find entity");
                if (await _db.DeleteAsync<Download>(d => d.Id.Equals(id) && d.CourseId.Equals(courseId)
